...
variables <- c("var_name",	"var_descr",	"Type",	"var_data_holding",	"var_ds"	,
               "var_links",	"var_lib",	"var_url", "Population", "DCE", 
               "official_lang_var_VMS", "official_lang_var_CP")


  metadata_import <- readr::read_delim("var_all_consolidated.csv", 
                                       delim = "|", escape_double = FALSE, trim_ws = TRUE)

metadata_lang_manual_check <- readr::read_csv("data_concordance_output_20240116.csv", 
...
metadata_raw <- dplyr::left_join(x = metadata_import,
...
readr::write_csv(metadata_raw, paste0("var-trimmed-", Sys.Date(),".csv"))
